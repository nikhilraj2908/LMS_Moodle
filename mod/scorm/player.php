    ...
    // move the Exit activity button into the resize-handles wrapper
    ...
    if (file_exists($CFG->dirroot.'/mod/scorm/datamodels/'.$scorm->version.'.php')) {
        ...
        include_once($CFG->dirroot.'/mod/scorm/datamodels/scorm_12.php');
            ...
            // move the Exit activity button into the TOC's resize-handles wrapper, retrying until it works
$PAGE->requires->js_init_code("
    require(['jquery'], function($) {
        var poll = setInterval(function() {
            var \$wrapper = $('.yui3-resize-handles-wrapper');
            // catch either the <a id='exitactivity'> (header) or the div.btn you saw in devtools
            var \$exit    = $('#exitactivity, .btn.btn-secondary:contains(\"Exit activity\")');
            if (\$wrapper.length && \$exit.length) {
                // move it into the empty wrapper you highlighted
                \$exit.appendTo(\$wrapper);
                // (optional) tweak its spacing so it looks nice
                \$exit.css({ display: 'block', margin: '10px auto' });
                clearInterval(poll);
            }
        }, 200);
    });
